...
class Admin extends Controller {
	...
	function exec($f3) {
		...
		elseif (!$f3->exists('POST.contents') ||
			...
			/* mogelijkheid om title te veranderen terwijl de url hetzelfde blijft */
			$slug=$f3->get('POST.old_slug');
			error_log("OLDSLUG=".$slug);
			if (empty($slug)) {
				error_log("new slug");
			...
			elseif ($f3->exists('POST.save')) {
				...
				// Replace data
				$page->copyfrom('POST');
				$f3->get('POST.deleted') ? $page->set('deleted',1) : $page->set('deleted',0);
				$f3->get('POST.available') ? $page->set('available',1) : $page->set('available',0);

				$date=strtotime($f3->get('POST.updated'));
				$page->set('slug',$slug);
				$page->set('title',$f3->get('POST.title'));
				error_log($slug."====".$f3->get('POST.title'));
				$page->set('updated', isset($date)?$date:time());
				$page->save();
			}
			// Redirect
			$f3->reroute('/admin/pages');
